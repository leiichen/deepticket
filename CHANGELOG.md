...
- **Ingress 外部契约精简**：`POST /api/ingress/events` 改为 `source`、`project_id`、`external_id`、`question`、可选 `image_urls` 与 `extensions`；Webhook / `GET /api/ingress/jobs/{id}` 返回 `reply` 与透传 `extensions`；按 `source` 匹配路由，按 `project_id` 加载项目代码与 MCP
