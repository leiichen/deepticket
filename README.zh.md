...
<p align="center">
  ...
  ·
  <a href="#内网部署踩坑实录"><strong>部署踩坑</strong></a>
</p>
...
DeepTicket 当前处于 **Alpha（v0.5.1）**。核心链路已经覆盖：项目管理、Git 知识库、Skill / MCP、OpenHands Agent、工作台对话、Ingress 异步分析、Webhook 回写和运行观测。

> **Java 版本（计划中）：** 面向更习惯 JVM 技术栈、内网 Maven/Nexus 部署的团队，Java/Spring 实现已在路线图中，将与本仓库一致的 Ingress 契约与项目模型对齐；进展请关注本仓库动态。
...
以下来自笔者和部分使用者的实际部署反馈，写给以 Java / Go / 运维为主、或不熟悉 Python 依赖与镜像构建的同学。

### 1. 内网源缺 wheel，构建可能拖到数小时

内网 PyPI / 制品库若没有同步 DeepTicket 所需的 OpenHands 依赖及其完整依赖链，或只有源码包而缺少目标环境的 wheel，`pip` 可能回退到源码构建。部分原生依赖还需要 C/C++、Rust 等工具链；在打包机上临时下载、编译，内网环境下可能耗时数小时甚至失败。

建议在与目标环境一致的构建机预先下载并验证完整 wheel，再同步到公司制品库；生产构建优先只安装 wheel，避免无意触发源码编译。可以将预构建镜像同步到内部镜像仓库，进一步减少部署时的外网与构建依赖。

### 2. CentOS 7 的 glibc 版本过低

CentOS 7 自带 glibc 2.17。若 OpenHands 依赖链中的 Linux 二进制包要求 `GLIBC_2.28` 或更高版本，安装或运行时会直接报错，补装 Python 包无法解决。

不要继续以 CentOS 7 作为构建或运行基础镜像；建议换用 Rocky Linux 9 等较新的企业发行版，或直接沿用本项目基于 Debian Bookworm 的镜像。遇到此类报错时，请以日志中实际提示的二进制包名为准排查。
